...
//  Photorama
//  Created by Kranthi Pyreddy on 4/8/21.
import UIKit
import CoreData
class TagsViewController: UITableViewController {
    ...
    //Setting the table view’s data source
    ...
    private func updateTags() {
        store.fetchAllTags {
            ...
            case let .success(tags):
                ...
                //Updating the selected index paths
                guard let photoTags = self.photo.tags as? Set<Tag>
                else {
                    return
                }
                for tag in photoTags {
                    if let index =
                        self.tagDataSource.tags.firstIndex(of: tag) {
    ...
    //Dismissing the tags view controller
